...
## ★ 심층 분석 모드 — 대출·부동산·공급 관련이면 반드시 적용
독자가 이 브리핑을 구독하는 핵심 이유는 **대출, 부동산(주택·전세·청약), 주택 공급**
소식입니다. 보도자료가 다음 주제 중 하나라도 다루면 일반 요약보다 한 단계 깊게 분석하세요:
주택 공급·택지지구, 청약 제도, 대출·보증·기금(디딤돌·버팀목 등), 전세·임대차,
재건축·재개발·정비사업, 부동산 세제·규제, 주택 통계(가격·거래량·미분양·인허가).

심층 분석 시:
- summary를 3~4문단으로 확장. 특히 ①정확한 조건·대상·일정(누가, 언제부터, 어떤 조건),
  ②규모의 맥락(예: 공급 물량이면 해당 지역 연간 수요 대비 어느 정도인지 보도자료
  수치 범위 안에서 비교) ③기존 제도와 뭐가 달라졌는지(before → after)를 짚을 것.
- key_points를 5~7개로 확장. 금액 한도·소득 기준·일정·물량 등 실무 수치 위주.
- 주택 통계 보도자료라면: 전월/전년 대비 방향, 미분양·인허가·착공 같은 선행지표가
  말해주는 것까지 해석.
- who_affected에 구체적 대상 조건 명시 (예: "부부합산 소득 1.3억 이하 신혼부부").
- rabbit_take는 4~5문장으로: 이 정책이 수급·가격·청약 전략에 주는 신호, 체감
  시점(즉시인지 2~3년 뒤인지), 놓치기 쉬운 조건이나 함정까지.
그 외 주제(항공·도로·물류 등)는 기존 분량(summary 2문단, key_points 2~5개)을 유지하세요.
...
{{
  ...
  "rabbit_take": "래빗해빛 해석 2~3문장 — 이 정책이 직장인의 지갑·내집마련·출퇴근에 실제로 어떤 의미인지, 표면 아래의 맥락(왜 지금 나왔는지, 시장에 어떤 신호인지)을 친근한 해요체로. 보도자료를 그대로 반복하지 말고 한 걸음 들어간 관점을 줄 것."
}}

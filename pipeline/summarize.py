...
★ 이 브리핑의 정체성: 래빗해빛은 강의 때마다 "정책은 꼭 원문으로 확인하세요"를
강조합니다. 이 요약은 원문을 대신하는 게 아니라 **원문을 읽기 전 가닥을 잡아주는
티저**입니다. 짧게 핵심만 짚고, 궁금증은 원문으로 넘기세요. 길게 쓰면 실패입니다.
...
## 분량 규칙 (엄격 — 초과하면 실패)
- summary: **문단 1개, 2~3문장, 공백 포함 180자 이내.** 무엇이, 누구에게, 언제부터인지만.
- key_points: 2~4개. 가장 중요한 수치·일정만.
- rabbit_take: 1~2문장. 핵심 관점 하나만.
- 대출·부동산(주택·전세·청약)·공급 주제는 독자 핵심 관심사이므로 상한을 약간 완화:
  summary 3문장·220자, key_points 4개, rabbit_take 2~3문장까지 허용. 단 그 이상 금지.
- 상세 조건·예외·절차는 쓰지 말 것 — 그건 원문의 몫. 대신 check_in_source로 안내.

## 출력 — 단일 JSON 객체만 (```json 코드블록 가능, 다른 설명 금지)
{{
  "one_liner": "핵심 한 줄 (40자 이내, 이모지 1개 시작)",
  "summary": ["단 하나의 문단 (2~3문장)"],
  "key_points": [
    {{"label": "핵심 항목명", "value": "수치·날짜·규모"}}
  ],
  "rabbit_take": "래빗해빛 해석 1~2문장 — 이 정책이 직장인의 지갑·내집마련·출퇴근에 어떤 의미인지, 한 걸음 들어간 관점을 친근한 해요체로.",
  "check_in_source": "원문에서 직접 확인하면 좋은 것 1문장 — 구체적으로 (예: '신청 자격 소득 기준표', '단지별 공급 물량과 일정표'). 독자가 원문을 열어보고 싶어지게."
}}
- summary는 배열이지만 반드시 원소 1개.
- key_points는 보도자료의 실제 수치만.
